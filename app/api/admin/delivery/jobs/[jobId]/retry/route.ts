...
import { retryDeliveryJob } from "@/lib/delivery/delivery-service";
...
export async function POST(request: NextRequest, { params }: RouteProps) {
  try {
    ...
    const limited = await checkRequestRateLimit({ request, keyPrefix: "admin-delivery-retry", limit: 30, windowSeconds: 60 });
    ...
    const job = await retryDeliveryJob(jobId, access.user.id);
