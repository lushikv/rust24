...
import { processDeliveryJobDryRun } from "@/lib/delivery/delivery-service";
...
export async function POST(request: NextRequest, { params }: RouteProps) {
  try {
    ...
    const limited = await checkRequestRateLimit({ request, keyPrefix: "admin-delivery-dry-run", limit: 30, windowSeconds: 60 });
    ...
    const result = await processDeliveryJobDryRun(jobId, access.user.id);
